...
export default function ShopPage() {
  ...
  const { data, error, isLoading } = useSWR("/api/memberData", fetcher);
  ...
  return (
    <div className="flex flex-col w-full">
      <h1 className="font-light text-2xl mt-12 mb-20 w-full text-center ">
        <span className="text-custom-green text-4xl font-bold">JKT48</span> Active Member
      </h1>
      {dataMember.length > 0 ? (
        <div className="flex flex-col">
          <div className="px-24 relative">
            <h1 className="text-2xl absolute font-light text-yellow-300">
              — Regular Member
            </h1>
            <h2 className="text-2xl absolute right-24 font-light text-yellow-300">
          ...
          <div className="mt-8 cards-member-container flex justify-center flex-wrap w-full relative">
            {memberRegular.map((member: memberType, i: number) => (
              <label key={i}>
                ...
                >
                  <div
                    className="front border-custom-green border rounded-2xl absolute top-0 bottom-0 left-0 right-0 overflow-hidden bg-zinc-950 hover:cursor-pointer"
                    style={{ backfaceVisibility: "hidden", transition: "0.6s" }}
                  >
                    <img
                      className="w-4/5 border-r border-custom-green"
                      src={
                        "/member/" +
                  ...
                  <div
                    className="back border-custom-green border rounded-2xl bg-zinc-950 flex flex-col justify-end items-center pb-3 absolute top-0 bottom-0 left-0 right-0 hover:cursor-pointer overflow-hidden"
                    style={{
          ...
          <div className="flex flex-col mt-24">
            <div className="px-24 relative">
              <h1 className="text-2xl absolute font-light text-pink-400">
                — Trainee Member
              </h1>
              <h2 className="text-2xl absolute right-24 font-light text-pink-400">
