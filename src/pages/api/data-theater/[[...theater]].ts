...
export default async function handler(
  ...
  if (req.query.theater?.[0]) {
    if (req.query.theater[0] === "add" && req.method === "POST") {
      ...
      await addData(ID, "theater", theater, ({status, message, data}: {status: boolean, message: string, data?: theaterType}) => {
    ...
    if (req.query.theater[0] === "update" && req.method === "PUT") {
      ...
      await updateData("theater", theater, ({status, message, data}: {status: boolean, message: string, data?: theaterType}) => {
        status ? res.status(200).json({status, message, data}) : res.status(400).json({status, message})
      })
    }
    ...
    if (req.query.theater[0] === "delete" && req.method === "DELETE") {
      ...
      await deleteData(id, "theater", description, ({status, message, data}: {status: boolean, message: string, data?: theaterType}) => {
        status ? res.status(200).json({status, message, data}) : res.status(400).json({status, message})
      })
    }    
